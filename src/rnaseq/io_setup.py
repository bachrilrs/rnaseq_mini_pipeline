...
def normalize_and_validate_counts(counts_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not all(
        pd.api.types.is_integer_dtype(counts_df[c]) for c in counts_df.columns
    ):  # ensure all columns are integer dtype
...
def normalize_and_validate_samples(
    samples_df: pd.DataFrame, counts_df: pd.DataFrame
) -> pd.DataFrame:
    """
...
def load_counts_tsv(
    file_path: str, pattern: str, sep="\t", gene_id_candidates=["EntrezGeneID", "GeneID", "gene_id"]
) -> pd.DataFrame:
    """
    ...
    for col in counts_df.columns:
        ...
        if not r_match:
            raise ValueError(
                f"The following sample {col} does not match the expected pattern {pattern}."
            )
...
def load_samples_csv(
    sample_file: str, counts_df: pd.DataFrame, separator: str = ","
) -> pd.DataFrame:
    samples_df = pd.read_csv(sample_file, sep=separator)
    return normalize_and_validate_samples(samples_df, counts_df)


def load_samples_geo_series(
    sample_file: str, counts_df: pd.DataFrame, samples_pattern: str = r'".*?"'
) -> pd.DataFrame:
    """
    ...
    if len(geo_accessions) != len(sample_ids):
        raise ValueError(
            "Number of GEO accessions does not match number of sample IDs "
            f"({len(geo_accessions)} vs {len(sample_ids)})"
        )
