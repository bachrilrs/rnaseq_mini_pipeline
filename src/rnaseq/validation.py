...
def validate_samples(df: pd.DataFrame, expected_conditions: set) -> None:
    ...
    if actual_conditions != expected_conditions:
        raise ValueError(
            f"Conditions in samples DataFrame do not match expected conditions. "
    ...
    if (n_per_cond < 2).any():
        raise ValueError(
            f"Each condition must have at least two samples. Found: {n_per_cond.to_dict()}"
        )
    ...
    for cond, sub in df.groupby("condition"):
        ...
        if replicates != expected:
            raise ValueError(
                f"Replicate numbers for condition '{cond}' must be {expected} "
                f"Found: {replicates}"
            )


def validate_data(
    counts_df: pd.DataFrame, samples_df: pd.DataFrame, expected_conditions: set
) -> None:
    """
    ...
    counts_samples = set(
        counts_df.columns
    )  # count column names must be sample_ids column in samples_df
