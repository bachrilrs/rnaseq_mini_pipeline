...
def count_zero_samples(df: pd.DataFrame , axis=0):
    """
    ...
    :rtype: int
    """
    count_zero_samples = (df.sum(axis=axis) == 0).sum()
    return count_zero_samples

def plot_count_zero_samples(counts_df: pd.DataFrame, output_file: str) -> None:
    """
    ...
    """
    zero_count = count_zero_samples(counts_df)
