...
class TestNormalizeAndValidateCounts:
    ...
    def test_counts_with_missing_gene_ids(self):
        ...
        df = pd.DataFrame(
            {"sample1": [10, 20, 30], "sample2": [5, 15, 25]}, index=["gene1", np.nan, "gene3"]
        )
        df.index.name = "gene_id"
    ...
    def test_counts_with_missing_sample_ids(self):
        ...
        df = pd.DataFrame(
            {"sample1": [10, 20, 30], np.nan: [5, 15, 25]}, index=["gene1", "gene2", "gene3"]
        )
        df.index.name = "gene_id"
    ...
    def test_counts_with_duplicate_sample_ids(self):
        ...
        df = pd.DataFrame(
            {"sample1": [10, 20, 30], "sample2": [5, 15, 25], "sample3": [8, 12, 22]},
    ...
    def test_load_counts_basic(self):
        ...
        try:
            result = load_counts_tsv(
                file_path=temp_file, pattern=r"(sample\d+)", sep="\t", gene_id_candidates=["GeneID"]
            )
    ...
    def test_load_counts_missing_gene_id_column(self):
        ...
        try:
            with pytest.raises(ValueError, match="No gene identifier column"):
                load_counts_tsv(file_path=temp_file, pattern=r"(sample\d+)", sep="\t")
        finally:
            os.unlink(temp_file)
    ...
    def test_load_counts_missing_pattern_none(self):
        ...
        try:
            with pytest.raises(ValueError, match="pattern must be provided"):
                load_counts_tsv(
                    file_path=temp_file, pattern=None, sep="\t", gene_id_candidates=["GeneID"]
                )
        finally:
            os.unlink(temp_file)
