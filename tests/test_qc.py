...
from rnaseq.qc import expressed_gene, library_size, log_transform, zero_fraction
...
class TestLibrarySize:
    ...
    def test_library_size_with_zeros(self):
        ...
        df = pd.DataFrame(
            {"sample1": [0, 20, 30], "sample2": [5, 0, 25]}, index=["gene1", "gene2", "gene3"]
        )
    ...
    def test_library_size_all_zeros(self):
        ...
        df = pd.DataFrame(
            {"sample1": [0, 0, 0], "sample2": [0, 0, 0]}, index=["gene1", "gene2", "gene3"]
        )
...
class TestZeroFraction:
    ...
    def test_zero_fraction_with_zeros(self):
        ...
        df = pd.DataFrame(
            {"sample1": [10, 0, 30], "sample2": [0, 0, 25]}, index=["gene1", "gene2", "gene3"]
        )
...
class TestExpressedGenes:
    ...
    def test_expressed_genes_basic(self):
        ...
        df = pd.DataFrame(
            {"sample1": [10, 0, 30], "sample2": [0, 15, 25]}, index=["gene1", "gene2", "gene3"]
        )
...
class TestLogTransform:
    ...
    def test_log1p_transform(self):
        ...
        df = pd.DataFrame({"sample1": [0, 1, np.e - 1]})
    ...
    def test_log10_transform(self):
        ...
        df = pd.DataFrame({"sample1": [0, 9, 99]})
